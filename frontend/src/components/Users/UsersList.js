...
const UsersList = ({
  ...
  const getRoleColor = (role) => {
    const colors = {
      ...
      DEVELOPER: 'bg-primary-100 text-primary-800 dark:bg-primary-900/20 dark:text-primary-400',
      QA: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400'
    };
    return colors[role] || 'bg-gray-100 text-gray-800 dark:bg-white/10 dark:text-gray-300';
  };
  ...
  const SortIcon = ({ column }) => {
    ...
    return filters.sortDir === 'asc' ? (
      <ArrowUpIcon className="h-4 w-4 text-primary-500 dark:text-primary-400" />
    ) : (
      <ArrowDownIcon className="h-4 w-4 text-primary-500 dark:text-primary-400" />
    );
  };
      ...
      <div className="card">
        <div className="animate-pulse p-6">
          <div className="flex gap-4 mb-6">
            ...
            <div className="h-10 bg-gray-200 dark:bg-white/10 rounded w-32"></div>
          </div>
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="h-16 bg-gray-200 dark:bg-white/10 rounded"></div>
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <input
              ...
              className="input-field placeholder-gray-500 dark:placeholder-gray-400"
            />
          </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
          <thead className="bg-gray-50 dark:bg-white/5">
            <tr>
              <th
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10"
                      ...
                      <div className="flex-shrink-0 h-10 w-10">
                        {user.profilePhoto && config.getStaticFileUrl(user.profilePhoto) ? (
                          <img
                            ...
                            className="h-10 w-10 rounded-full object-cover border border-gray-200 dark:border-white/10"
                          />
                        ) : (
                          <div className="h-10 w-10 rounded-full bg-gray-300 dark:bg-white/10 flex items-center justify-center">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="text-primary-600 dark:text-primary-400 hover:text-primary-900 dark:hover:text-primary-300 p-2 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors duration-200"
                        title="Edit user"
                      >
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-white/10 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 border border-gray-300 dark:border-white/10 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-white/10 text-gray-700 dark:text-gray-300"
            >
              Previous
            </button>
